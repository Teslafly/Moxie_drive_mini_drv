...
def replicate_sheet_trackst(fromnet, tonet, offset):
    ...
    for zone in fromzones:
        #print("REMOVE WHEN FIX ZOME COPY!!!")
        #continue
...
def place_instances(mainref, pitch, blacklist = None):
    ...
    basepositions = {}
    modules = sheetinstance.getChildren()
    #modules = list(set(modules)-set(blacklist))
    for m in modules:
        if m.GetReference() in blacklist:
            print("removing"+m.GetReference())
            modules.remove(m)
    ...
    for idx, si in enumerate(arrayedsheets, start=-instnum):
        ...
        for peer in si.getChildren():            
            ...
            if childid not in basepositions:
                continue
...
place_instances("Q701", (0, -14), blacklist=blacklist)
